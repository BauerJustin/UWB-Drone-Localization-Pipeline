...
class OutlierRejection:
    ...
    def _replace_outlier(self, plotted_values, outlier_measurements):
        ...
        data[-1, :][(data[-1, :] < const.NON_OUTLIER_MIN) | (data[-1, :] > const.NON_OUTLIER_MAX)] = np.nan
    ...
    def filter_outlier(self, incoming_value, active, plotted_values):
        ...
        if any(val < const.NON_OUTLIER_MIN or val > const.NON_OUTLIER_MAX for val in new_val):
            ...
            if self.is_active:
                if self.outlier_replacement:
                    ...
                    new_val_list = [None if val < const.NON_OUTLIER_MIN or val > const.NON_OUTLIER_MAX else val for val in new_val_list]
